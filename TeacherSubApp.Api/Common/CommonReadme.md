...
- `Result.Success()` / `Result.Failure(type, error)` are the only ways to
  construct a `Result`. `Failure` throws `InvalidOperationException` if you
  pass `ErrorType.None` or `Error.None()` — a failed result without a real
  error and a real category is a contradiction, and we fail fast on that at
  construction time rather than letting it reach a controller.
- `Result<T>` adds `Value`, returned on the success path. Its own
  `Failure(...)` re-validates the same invariants independently (it does
  not just delegate to the base, because `Result<T>`'s constructor is
...
- **`DbUpdateConcurrencyException` is not special-cased.** It currently
  ...
  - **`500 Internal Server Error`** 
    This is preserved for the GlobalExceptionHandler's catch-all branch, which is the only place that
    returns a `TraceId`. A concurrency failure is *expected* (the user can
    retry), so it should not return a `TraceId` — the frontend does not
    need to investigate server logs for this, and the user can recover
    without contacting support. If you want to log it, log it as a warning
    (like `DbUpdateException`) rather than an error.
