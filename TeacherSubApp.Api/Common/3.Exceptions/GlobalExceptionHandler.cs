...
namespace TeacherSubApp.Api.Common.Exceptions
{
    public sealed class GlobalExceptionHandler : IExceptionHandler
    {
        ...
        {
            _logger.LogWarning(exception, "Database error occurred. Path: {Path} | TraceId: {TraceId}", httpContext.Request.Path, httpContext.TraceIdentifier);

            ErrorResponse dbError = ErrorResponse.FromError(GlobalErrors.DatabaseError, httpContext.TraceIdentifier);
        ...
        {
            _logger.LogError(exception, "System crash. Path: {Path} | TraceId: {TraceId}", httpContext.Request.Path, httpContext.TraceIdentifier);

            ErrorResponse crashError = ErrorResponse.FromError(GlobalErrors.InternalServerError, httpContext.TraceIdentifier);
            httpContext.Response.StatusCode = StatusCodes.Status500InternalServerError;
